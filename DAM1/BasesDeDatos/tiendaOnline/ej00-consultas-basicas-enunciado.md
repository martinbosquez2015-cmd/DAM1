...
```
mysql> select * from pedidos order by fecha_pedido desc;
+-----------+------------+---------------------+-----------+-------------+
| id_pedido | id_cliente | fecha_pedido        | estado    | coste_total |
+-----------+------------+---------------------+-----------+-------------+
|        47 |          5 | 2025-09-08 22:58:53 | entregado |      258.99 |
...
```
mysql> select * from clientes order by fecha_registro asc limit 5;
+------------+----------------+-------------------+----------------+-----------+
| id_cliente | nombre         | email             | fecha_registro | pais      |
+------------+----------------+-------------------+----------------+-----------+
|          1 | Ana Torres     | ana@example.com   | 2023-01-10     | España    |
...
```
*se comola la opción asc debido a mejorar la claridad de lo que se busca a través del código*

### 3.4. Mostrar los 3 últimos pedidos realizados
```
mysql> select * from pedidos order by fecha_pedido desc limit 3;
+-----------+------------+---------------------+-----------+-------------+
| id_pedido | id_cliente | fecha_pedido        | estado    | coste_total |
+-----------+------------+---------------------+-----------+-------------+
|        47 |          5 | 2025-09-08 22:58:53 | entregado |      258.99 |
...
```
mysql> select length ('nombre') from clientes;
+-------------------+
| length ('nombre') |
+-------------------+
|                 6 |
...
```
mysql> select count(estado) from pedidos;
+---------------+
| count(estado) |
+---------------+
|           155 |
...
1 row in set (0,00 sec)
```
**Si se consideran los pedidos realizados como los que no han sido cancelados entonces:**
```
mysql> select count(estado) from pedidos where estado!='cancelado';
+---------------+
| count(estado) |
+---------------+
|           133 |
...
```
mysql> select avg(precio) as promedio_precio_prod from productos;
+----------------------+
| promedio_precio_prod |
+----------------------+
|           127.684667 |
+----------------------+
1 row in set (0,00 sec)
...
```
mysql> select precio from productos order by precio asc limit 1;
+--------+
| precio |
+--------+
|   9.99 |
...
```
mysql> select precio from productos order by precio desc limit 1;
+--------+
| precio |
+--------+
| 750.00 |
...
```
mysql> select upper (nombre) from productos;
+---------------------------+
| upper (nombre)            |
+---------------------------+
...
```
mysql> select lower(nombre) from clientes;
+--------------------+
| lower(nombre)      |
+--------------------+
| ana torres         |
...
```
mysql> SELECT SUBSTRING(nombre, 1, 3) AS Productos_3_caracteres FROM productos;
+------------------------+
| Productos_3_caracteres |
+------------------------+
| Lap                    |
| Sma                    |
| Aur                    |
...
```
mysql> select substring(nombre,-3) as 3_ultimos_caracteres from productos;
+----------------------+
| 3_ultimos_caracteres |
+----------------------+
| ron                  |
| omi                  |
| oth                  |
...
```
mysql> select now() as fecha_y_hora_actual;
+---------------------+
| fecha_y_hora_actual |
+---------------------+
| 2025-09-24 09:38:11 |
...
```
mysql> select nombre, timestampdiff (day, fecha_registro, CURDATE()) AS dias_desde_registro FROM clientes;
+--------------------+---------------------+
| nombre             | dias_desde_registro |
+--------------------+---------------------+
| Ana Torres         |                 988 |
| Luis Méndez        |                 952 |
| Carla Gómez        |                 919 |
